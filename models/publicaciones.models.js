...
export const AddPublicacionModel = async ({usuario_id, titulo, descripcion, img, precio, status}) => {

    const result = await pool.query(
        'INSERT INTO publicaciones (usuario_id, titulo, descripcion, img, precio, status) VALUES ($1 ,$2 ,$3 ,$4 ,$5 ,$6);',
        [usuario_id, titulo, descripcion, img, precio, status]);
...
export const getPublicacionByIdUsuarioModel = async (id) => {
    const result = await pool.query(
        'SELECT titulo, descripcion, img, precio, status FROM publicaciones WHERE id = $1;', [id]);
    //console.log(result);
    return result.rows[0]
}
